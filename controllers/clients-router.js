...
router.post('/', async (req, res) => {
  const { phone_num, isActive, workflow_id } = req.body;
  ...
  try {
    ...
    res.status(500).json({ err: 'The post could not be added at this time.' });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (deleteClient)
      res
        .status(200)
        .json({ message: 'You have successfully deleted the Client' });
  } catch (error) {
